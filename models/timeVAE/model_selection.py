# The model_selecetion.py file is a modification of test_vae.py.
...
if __name__ == '__main__':
    ...
    # read data    
    #valid_perc = 0.1
    train_input_file = f'{dataset_name}_train_U_valid.npz'
    train_data = utils.get_training_data(data_dir + train_input_file)
    N_t, T_t, D_t = train_data.shape   
    print('train_U_valid data shape:', N_t, T_t, D_t)
    ...
    configurations = [([100, 200, 400], 4, 2, 0.001), 
                    ([50, 100, 200], 4, 2, 0.001), 
                    ([100, 200, 400], 4, 6, 0.001),
                    ([100, 200, 400], 12, 2, 0.0001),
                    ([75, 150, 300], 4, 2, 0.001),
                    ([100, 200, 400], 4, 4, 0.001),
                    ([100, 200, 400], 8, 4, 0.0001),
                    ([100, 200, 400], 8, 4, 0.001),
                    ([75, 150, 300], 4, 6, 0.001)]
    
    # Re-train the selected model configurations:
    for configuration in configurations:
        
        ls, ld, tp, lr = configuration
        ...
        # save model 
        model_dir = './model/'
        signiture = f"{dataset_name}_{ls}_{ld}_{tp}_{str(lr)}_test"
        ...
        # Generate plots:
        plot_path = "./outputs/model_selection/plots/"
        ...
        output_dir = './outputs/model_selection/data'
        ...
        np.savez_compressed("../TSGBench/data/timevae/model_selection/" + sample_fname, data=samples)
